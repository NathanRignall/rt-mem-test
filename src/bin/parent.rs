...
fn main() {
    ...
    // open shared memory
    let send_shmem = ShmemConf::new().size(4096).flink("s_event_mapping").create().unwrap();
    let recv_shmem = ShmemConf::new().size(4096).flink("r_event_mapping").create().unwrap();

    // create event in shared memory
    let (send_event, _) = unsafe { raw_sync::events::Event::new(send_shmem.as_ptr(), true).unwrap() };
    let (recv_event, _) = unsafe { raw_sync::events::Event::new(recv_shmem.as_ptr(), true).unwrap() };
    ...
    loop {
        ...
        let timestamp = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_micros() as u64;
        times.push((timestamp, last_sleep.as_micros() as u64, last_duration.as_micros() as u64, overruns));
